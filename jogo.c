...
bool verifica_vitoria(Nanograma *nanograma, Celula tabuleiro[][COLUNAS])
{
    ...
    for (i = 0; i < LINHAS; i++)
    {
        ...
        if (pos_array != nanograma->quant_numeros_linha[i])
            return false;
        for (contador = 0; contador < pos_array; contador++)
        {
            if (nanograma->numeros_linha[i][contador] != celulas_clicadas[contador])
                return false;
        }
    }
    ...
    for (j = 0; j < COLUNAS; j++)
    {
        ...
        if (pos_array != nanograma->quant_numeros_coluna[j])
            return false;
        for (contador = 0; contador < pos_array; contador++)
        {
            if (celulas_clicadas[contador] != nanograma->numeros_coluna[j][contador])
                return false;
        }
    }
    return true;
...
void desenha_menu(Tela *t, Lista_Botao *button)
{
    Ponto txt_nanograma = {(LARGURA_JANELA / 2) - 100, (ALTURA_JANELA / 2) - 100};
    escreve_texto(t, txt_nanograma, "NANOGRAMA");
...
void desenha_texto_celulas_restantes(Tela *t, Nanograma *nanograma, Celula tabuleiro[][COLUNAS])
{
    ...
    calcula_celulas_restante(nanograma, tabuleiro);
    int celulas_restantes = nanograma->quant_celulas_pintar - nanograma->quant_celulas_pintadas;
    Ponto txt_ganhou = {LARGURA_JANELA - 250, 40};

    escreve_texto(t, txt_ganhou, "Células a pintar :");

    sprintf(txt_qnt_celul_pint, "%d", celulas_restantes);
            ...
            mouse.y >= button->lista_btn[i].pos.y && mouse.y <= button->lista_btn[i].tam.alt + button->lista_btn[i].pos.y)
        {
            ...
            case MENU:
                ...
                *menu_show = true;
                break;
            case DESFAZER:
                tabuleiro[nanograma->jogada.nova_jogada_i][nanograma->jogada.nova_jogada_j].estado = VAZIO;
                nanograma->jogada.nova_jogada_i = -1;
                nanograma->jogada.nova_jogada_j = -1;
                nanograma->jogada.jogada_passada_j = -1;
                nanograma->jogada.jogada_passada_i = -1;
                break;
            case DICA:
...
void atualiza_historico_jogada(Nanograma *nanograma, int i, int j)
{
    ...
    {
        nanograma->jogada.jogada_passada_i = 0;
        nanograma->jogada.jogada_passada_j = 0;
        nanograma->jogada.nova_jogada_i = i;
        nanograma->jogada.nova_jogada_j = j;
    }
    else
    {
        nanograma->jogada.jogada_passada_i = nanograma->jogada.nova_jogada_i;
        nanograma->jogada.jogada_passada_j = nanograma->jogada.nova_jogada_j;
        nanograma->jogada.nova_jogada_i = i;
        nanograma->jogada.nova_jogada_j = j;
    }
}
